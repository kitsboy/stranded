...
export default function CaseStudyExport({ site, liveBtc = 85_000 }: { site: CaseStudySite; liveBtc?: number }) {
  ...
  return (
    <div className="flex gap-2" data-testid="case-study-export">
      <button type="button" onClick={md} className="flex-1 rounded-lg border border-white/15 py-1.5 text-label hover:bg-white/5">
        Case study .md
      </button>
      <button type="button" onClick={html} className="flex-1 rounded-lg border border-[#5BC0BE]/30 py-1.5 text-label text-[#5BC0BE] hover:bg-[#5BC0BE]/10">
        Case study HTML
      </button>
    </div>
  )
