...
export default function RoiProjectionChart({ dailyBtc, btcUsd, years = 8 }: Props) {
  ...
  return (
    <div className="space-y-2">
      <div className="text-[10px] uppercase tracking-wider text-gray-400">Multi-year revenue (halving-adjusted)</div>
        ...
        {rows.map(r => {
          ...
          return (
            <div key={r.year} className="flex-1 flex flex-col items-center gap-1" title={`${r.year}: $${Math.round(usd).toLocaleString()}`}>
              ...
              <span className="text-[8px] text-gray-400 font-mono">{String(r.year).slice(2)}</span>
            </div>
          )
        })}
      </div>
      <div className="text-[10px] text-gray-400">Assumes constant hashrate; reward halves per schedule</div>
    </div>
  )
}