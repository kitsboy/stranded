...
/** Scale enrichment-time USD potential (pegged at $85k BTC) to a live price. */
export function scalePotentialUsd(baseUsd: number, liveBtc = 85000, baseBtc = 85000): number {
  if (!baseBtc || !isFinite(liveBtc)) return baseUsd
  return Math.round(baseUsd * (liveBtc / baseBtc))
}

export function portfolioDailyPotentialUsd(sites: EnrichedSite[], liveBtc = 85000): number {
  return sites.reduce((sum, s) => sum + scalePotentialUsd(s.potentialDailyProfitUsd, liveBtc), 0)
}
...
export function exportPortfolioCsv(sites: EnrichedSite[], liveBtc = 85000): string {
  const headers = ['id', 'name', 'province', 'emission_kg_day', 'stranded_score', 'genset_kw', 'potential_daily_usd', 'btc_price']
  const rows = sites.map(s => [
    ...
    scalePotentialUsd(s.potentialDailyProfitUsd, liveBtc),
    liveBtc,
  ])
...
export function exportPortfolioPdfHtml(sites: EnrichedSite[], liveBtc: number): string {
  ...
  const totalPot = portfolioDailyPotentialUsd(sites, liveBtc)
  const rows = sites.map(s =>
...
<p><strong>Total emission:</strong> ${escapeHtml(totalEm.toLocaleString())} kg/day · <strong>Daily potential:</strong> $${escapeHtml(totalPot.toLocaleString())} (optimistic scenario @ live BTC)</p>
<table><thead><tr><th>Site</th><th>Province</th><th>kg/day</th><th>Score</th></tr></thead><tbody>${rows}</tbody></table>
<p style="margin-top:30px;font-size:11px;color:#666">Not financial advice. Optimistic-scenario figures at the live BTC price, not a verified forecast. Data: ECCC open dataset.</p>
</body></html>`
}